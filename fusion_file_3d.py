...
class BuildDataset(Dataset):
    def __init__(self, data_all, num_target=15):
        ...
        for index, data in data_all.iterrows():
            ...
            condition = np.concatenate(([data['W'], data['V'], data['rmsa'], data['D'], data['Vx'], data['Vz']], [userage_normalized], gender_one_hot))
            condition_all.append(condition)
            points = []
            
            rmsa_all.append(data['rmsa'])
            acc_all.append(data[32:56])
            vib_all.append(data[56:])
            for i in range(num_target):
                cur_pos = np.array([data[f'{i}_touch_x'], data[f'{i}_touch_y'], data[f'{i}_touch_z']])
                points.append(cur_pos)
            points = np.array(points).transpose(0, 1)  # (num_target, 2)
...
def split_train_test_few(data_condition, num_shot=1, seed=0, test_ratio=0.4, val_ratio=0.1):
    ...
    user_list = data_condition['username'].unique()
        ...
        for w in W_values:
            for v in V_values:
                ...
                if data_subset.shape[0] == 0:
                    print(f'No data for user {user}, W {w}, V {v}')
                train_data_candidate, temp_data = train_test_split(data_subset, test_size=test_ratio + val_ratio)
                
                # Handle small temp_data cases
                if len(temp_data) <= 3:
                    if len(temp_data) == 3:
                        val_data = temp_data.iloc[:1]  # val gets 1 sample
                        test_data = temp_data.iloc[1:]  # test gets 2 samples
                    elif len(temp_data) == 2:
                        val_data = temp_data.iloc[:1]  # val gets 1 sample  
                        test_data = temp_data.iloc[1:]  # test gets 1 sample
                    else:  # len(temp_data) == 1 or 0
                        print('split data error')
                else:
...
class FusionModel(nn.Module):
    def __init__(self, paras, args):
        ...
        self.adapted_paras = nn.Parameter(self.base_paras.clone()+1e-6, requires_grad=True)
        self.weights_expert = torch.tensor([1.,0.,0.,0.]).float().to(device)
    ...
    def calc_mu_sigma_3d(self, para, x):
        # Get batch size
        bs = x.shape[0]
        
        # Repeat para to match batch size
        #para = para.repeat(bs, 1,1)
        
        # Extract the x values
        x1, x2, x3, x4, x5, x6 = x[:, 0].unsqueeze(1), x[:, 1].unsqueeze(1), x[:, 2].unsqueeze(1), \
                                x[:, 3].unsqueeze(1), x[:, 4].unsqueeze(1), x[:, 5].unsqueeze(1)
        # Compute mu_x, mu_y, mu_z
        mu_x = para[:, :, 0] + para[:, :, 1] * x2 + para[:, :, 2] * x1 + para[:, :, 3] * x3 + para[:, :, 4] * x5 + \
            para[:, :, 5] * x5 / x4 + para[:, :, 6] * 2 * torch.asin(x1 / (2 * x4))
        
        mu_y = torch.zeros(bs, para.shape[1], device=para.device)
        
        mu_z = para[:, :, 7] + para[:, :, 8] * x2 + para[:, :, 9] * x1 + para[:, :, 10] * x3 + para[:, :, 11] * x5 + \
            para[:, :, 12] * x6 / (x4 ** 2) + para[:, :, 13] * 2 * torch.asin(x1 / (2 * x4))
        
        # Compute sigma_x, sigma_y, sigma_z
        sigma_x = torch.sqrt(torch.abs(para[:, :, 14]) + torch.abs(para[:, :, 15]) * x5 ** 2 + torch.abs(para[:, :, 16]) * x2 ** 2 + \
                            torch.abs(para[:, :, 17]) * x1 ** 2 + torch.abs(para[:, :, 18]) * (x2 / x1) + \
                            torch.abs(para[:, :, 19]) * (x5 / x1)** 2 + torch.abs(para[:, :, 20]) * x3 ** 2 + \
                            torch.abs(para[:, :, 21]) * x4 ** 2 + torch.abs(para[:, :, 22]) * (x5 / x4) ** 2 + \
                            torch.abs(para[:, :, 23]) * 2 * torch.asin(x1 / (2 * x4))** 2)
        
        sigma_y = torch.sqrt(torch.abs(para[:, :, 24]) + torch.abs(para[:, :, 25]) * x2 ** 2 + torch.abs(para[:, :, 26]) * x1 ** 2 + \
                            torch.abs(para[:, :, 27]) * x3 ** 2 + torch.abs(para[:, :, 28]) * x4 ** 2 + \
                            torch.abs(para[:, :, 29]) * (2 * torch.asin(x1 / (2 * x4))) ** 2)
        
        sigma_z = torch.sqrt(torch.abs(para[:, :, 30]) + torch.abs(para[:, :, 31]) * x6 ** 2 + torch.abs(para[:, :, 32]) * x2 ** 2 + \
                            torch.abs(para[:, :, 33]) * x1 ** 2 + torch.abs(para[:, :, 34]) * x4 ** 2 + \
                            torch.abs(para[:, :, 35]) * (x6 / x1) + torch.abs(para[:, :,36]) * (x6 / (x4 ** 2)) ** 2 + \
                            torch.abs(para[:, :, 37]) * 2 * torch.asin(x1 / (2 * x4))** 2)
        
        # Stack the mu values
        MU = torch.stack((mu_x, mu_y, mu_z), dim=2)
        
        # Build the sigma matrix
        SIGMA = torch.zeros(bs, para.shape[1], 3, 3, device=para.device)
        SIGMA[:, :, 0, 0] = sigma_x ** 2
        SIGMA[:, :, 1, 1] = sigma_y ** 2
        SIGMA[:, :, 2, 2] = sigma_z ** 2
    ...
    def forward_expert(self, condition, touch_xy, env=None):
        ...
        param_cur = self.base_paras[self.weights_expert==1]
        param_cur = param_cur.unsqueeze(0).expand(size_batch, -1, -1)
        MU, SIGMA = self.calc_mu_sigma_3d(param_cur, condition)
        ...
        for cur_target in range(self.num_target):
            ...
            #jitter = 1e-6 * torch.eye(3, device=SIGMA.device)
            #jitter = jitter.unsqueeze(0).unsqueeze(1).repeat(size_batch, 1, 1, 1)
            SIGMA += 1e-6
    ...
    def forward(self, condition, touch_xy, env=None):
        # condition (W, V, gesture, age, gender): (bs, 5)
        # touch_xy (touch_x, touch_y): (bs, self.num_target , 2)
        # env: env input
        # self.paras: (self.num_expert, self.dim_expert)
        condition = condition.float()
        condition_touch = condition[:, :6]
        condition_user = condition[:, 6:]
        size_batch = touch_xy.shape[0]
        ...
        MU, SIGMA = self.calc_mu_sigma_3d(adapted_paras, condition_touch)
        # Context-aware fusion weights with diversity regularizationf
        ...
        for cur_target in range(self.num_target):
            ...
            jitter = 1e-6 * torch.eye(3, device=SIGMA.device)
            jitter = jitter.unsqueeze(0).unsqueeze(1).repeat(size_batch, self.num_expert, 1, 1)
            SIGMA_regularized = SIGMA + jitter
            
            mix = torch.distributions.Categorical(probs=fusion_weight)
            comp = torch.distributions.MultivariateNormal(
                loc=MU,
                covariance_matrix=SIGMA_regularized
            )
...
para = pd.read_csv('./dataset_3d/parameters.csv')
para = para.drop(para.columns[0], axis=1)
endpoints = pd.read_csv('./dataset_3d/endpoints_total.csv')
rmsa = pd.read_csv('./dataset_3d/rmsa_total.csv')
acc = pd.read_csv('./dataset_3d/acc_total.csv')
vib = pd.read_csv('./dataset_3d/vib_total.csv')
        ...
        if patience_counter >= early_stop_patience or epoch == args.epochs - 1:
            ...
            for i, (condition, touch_xy, env, label) in enumerate(dataloader_test):
                with torch.no_grad():
                    ...
                    # save error data
                    error_condition.append(condition.cpu())
                    error_touch.append(touch_xy.cpu())
                    error_acc.append(acc.cpu())
                    error_vib.append(vib.cpu())
                    error_label.append(label.cpu())
                    error_prob.append(prob.cpu())
                    error_weight.append(model.fusion_weight.cpu())
